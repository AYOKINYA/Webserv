...
struct {//구조체 
	...
	{(char *)"gif", (char *)"image/gif" },  //gif 
	{(char *)"jpg", (char *)"image/jpg"},    //jpg
	{(char *)"jpeg",(char *)"image/jpeg"},   //jpeg
	{(char *)"png", (char *)"image/png" },  //png
	{(char *)"htm", (char *)"text/html" },  //htm
	{(char *)"html",(char *)"text/html" },  //html
	{0,0} };//NULL
...
int main(int argc, char *argv[])//메인함수 
{
	...
	//printf("start\n");
	close(ff);//로그파일을 닫아준다. 
	int port, listenfd, socketfd, hit;//int형으로 선언 
	...
	for(hit=1;;hit++){//while문과 동일 
		...
		if((pid = fork()) < 0) {//안열리면 
			...
			if(pid == 0) {////////////////////fork시작 
				...
				int file_fd, buflen, len;//int형변수 선언 
